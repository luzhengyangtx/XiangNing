...
export async function fetchInventory(params?: {
  search?: string; category?: string; sortCol?: string; sortDir?: string;
...
export async function adjustStockBatch(items: { productId: string; delta: number }[], reason?: string) {
...
export async function getCurrentUser() {
  ...
  const d = await res.json(); return d.user;
}
